...
class ProductPage(BasePage):
    def add_item_to_basket(self): # метод добавления товара в корзину
        button_add_to_basket = self.browser.find_element(*ProductPageLocators.ADD_TO_BASKET_BUTTON)
        button_add_to_basket.click()

    def check_add_item_to_basket(self): # Проверка: товар добавлен в корзину
        self.message_items_should_be_add_to_basket()
        self.cost_should_be_eql_price()

    def message_items_should_be_add_to_basket(self): # Проверка: название товара в сообщении совпадает с добавленным товаром
    ...
    def cost_should_be_eql_price(self): # Проверка: стоимость корзины равна цене товара
    ...
    def should_not_be_success_message(self): # Проверка, что элемент не появился на странице
        assert self.is_not_element_present(*ProductPageLocators.SUCCESS_MESSAGE), \
            "Success message is presented, but should not be"

    def should_be_success_message(self): # Проверка, что элемент не исчез, но должен был
